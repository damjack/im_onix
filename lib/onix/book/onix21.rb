...
module Onix
  module Book
    module Onix21
      class ShortToRef
        def self.names
          @shortnames ||= YAML.load(File.read(File.dirname(__FILE__), "data", "onix21", "shortnames.yml"))
        end
      end
      ...
      class SubsetDsl < Onix::Book::Model::SubsetDsl
        ...
        def self.ref_to_short(name)
          Onix::Book::Onix21::RefToShort.names[name]
        end

        def self.get_class(name)
          if Onix::Book::Onix21.const_defined?(name,false)
            Onix::Book::Onix21.const_get(name)
          else
